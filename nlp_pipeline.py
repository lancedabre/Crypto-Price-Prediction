...
def get_daily_sentiment(
    ...
    if not headlines:
        return pd.DataFrame(columns=["Sentiment_Score"]).astype({"Sentiment_Score": float})

    rows = []
    for item in headlines:
        sentiment = analyzer.polarity_scores(item["title"])
        rows.append({"Published": item["published"], "Sentiment_Score": sentiment["compound"]})

    df = pd.DataFrame(rows)
    df["Published"] = pd.to_datetime(df["Published"], utc=True, errors="coerce")
    df.index = df["Published"].dt.tz_localize(None).dt.floor("D")
    df.index.name = "Date"

    daily_avg = df.groupby(df.index)["Sentiment_Score"].mean()
